---
title: "Math 300 Lesson 15 Notes"
subtitle: "Multiple Regression - Two Numerical"
...
Work through the learning checks LC6.2 - 6.3. Complete the code as necessary.  
...
#  select(ID, debt = ________, credit_limit = Limit, 
#         income = _______, credit_rating = ____________, age = Age)
```
...
# Complete code and remove comment symbol
#credit_ch6 %>% 
#  select(debt, ___________, income) %>% 
#  my_skim() %>%
#  print()
```
...
**(LC 6.2)** Conduct a new exploratory data analysis with the same outcome variable $y$ being `debt` but with `credit_rating` and `age` as the new explanatory variables $x_1$ and $x_2$. Remember, this involves three things:
...
```{r message=FALSE}
# Complete code and remove comment symbol
# ggplot(credit_ch6, aes(x = ____________, y = debt)) +
#   geom_point() +
#   labs(
#     x = "Credit rating", y = "Credit card debt (in $)",
#     title = "Debt and credit rating"
#   ) +
#   geom_smooth(method = "lm", se = ________)
```


```{r message=FALSE}
# Repeat for age and debt
...
**(LC 6.3)** Fit a new simple linear regression using `lm(debt ~ credit_rating + age, data = credit_ch6)` where `credit_rating` and `age` are the new numerical explanatory variables $x_1$ and $x_2$. Get information about the "best-fitting" regression plane from the regression table by applying the `get_regression_table()` function. How do the regression results match up with the results from your previous exploratory data analysis? 

```{r}
# Complete code and remove comment symbol
# Fit regression model:
# debt_model_2 <- lm(_________ ~ ___________ + _________, data = credit_ch6)
```
...
# Complete code and remove comment symbol
# get_regression_table(_______________)
``` 
