...
+ The central limit theorem is difficult. The key idea is that if we create a point estimate that is using summation, then the **distribution of this point estimate** tends to be normal. The larger he sample, the closer the distribution is to being normal. If we are finding a mean, then the larger the sample, the smaller the standard error.
...
Notice how this distribution of is approximately normal. That is the central limit theorem. If we used 10 points per sample instead of 4, the standard error would be smaller. 
